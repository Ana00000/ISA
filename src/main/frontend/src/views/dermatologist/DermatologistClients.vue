    ...
    <div style="
        ...
        <div class="searchDiv">
            ...
            <v-container>
                ...
                <v-layout row wrap>
                    <v-card
                        style="width: 55%; height: 300px; overflow-y: scroll" class="allExams">
                        <v-toolbar
                        color="#13077d" dark>
                        </v-toolbar>
                        <v-list two-line>
                        <v-list-item-group
                            ...
                            <v-list-item :key="examination.id" :value="examination">
                                <template>  
                                <v-list-item-content>
                                    <v-list-item-subtitle v-text="'Appointment id: '+examination.id + ', Patient: ' +examination.patient.name+' '+examination.patient.lastName"/>
                            ...
                            <v-divider v-if="`C-${id}` <examinations.length-1" :key="`C-${id}`"/>
                            </template>
                        </v-list-item-group>
                        </v-list>
                    </v-card>
...
export default {
    ...
    methods: {
      ...
      getClients() {
          this.$http.get('http://localhost:8081/appointments/patientsFromDoneAppointmentsByDoctor/' + this.dermatologist.id)
            .then(res => {
      ...
      getExams() {
         axios.get('http://localhost:8081/appointments/appointmentsOfPatientsByDoctor/' + this.dermatologist.id)
                .then(res => {
